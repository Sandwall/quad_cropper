...
// wraps both behaviors in a single function, controlled with `spacesToTabs`
// if spacesToTabs is false, the function instead converts tabs to spaces
void tab_convert(EditorCtx* ctx, int spacesToTabs) {
	Temp scratch = scratch_begin(NULL);
	EditorCursorArray cursors;
	ed_cursor_ranges(scratch.arena, ctx, &cursors);
	EditorBatchEdit editorBatchEdit;
	ed_edit_batch_begin(ctx, &editorBatchEdit);

	int iterateOverEntireFile = 1;
	for EachIndex(i, cursors.size) {
		// if a single cursor has a nonempty selection, then we'll iterate over those selections
		// instead over the entire file
		if(cursors.array[i].sel.first_off != cursors.array[i].sel.last_off)
			iterateOverEntireFile = 0;
	}

	if(!iterateOverEntireFile) {
		EditorBatchReplace replacements;
		for EachIndex(i, cursors.size) {
			uint64_t startLine = ed_line_at_offset(ctx, cursors.array[i].sel.first_off);
			uint64_t endLine = ed_line_at_offset(ctx, cursors.array[i].sel.last_off);

			replacements.size = endLine - startLine + 1;
			replacements.array = push_array(scratch.arena, EditorReplaceData, replacements.size);

			// make a replacement for each line
			for EachIndex(j, replacements.size) {
				EditorOffsetRange lineRange;
				String8 lineText;
				ed_byte_range_at_line(ctx, startLine + j, &lineRange);
				ed_string_at_range(scratch.arena, ctx, &lineRange, &lineText);

				int numTabs = 0, numSpaces = 0, startOfLine = 0;

				// first count the current leading tabs and spaces
				for EachIndex(k, lineText.size) {
					if('\t' == lineText.str[k]) numTabs++;
					else if (' ' == lineText.str[k]) numSpaces++;
					else break;
					startOfLine++;
				}

				// now compute desired leading tabs and spaces
				if(spacesToTabs) {
					numTabs += numSpaces / TABSTOP;
					numSpaces = numSpaces % TABSTOP;
				} else {
					numSpaces += numTabs * TABSTOP;
					numTabs = 0;
				}

				char* newLeadingData = push_array(scratch.arena, char, numTabs + numSpaces);
				memset(newLeadingData, '\t', numTabs);
				memset(newLeadingData + numTabs, ' ', numSpaces);
				replacements.array[j].buf = str8(newLeadingData, numTabs + numSpaces);
				replacements.array[j].range.first_off = lineRange.first_off;
				replacements.array[j].range.last_off = lineRange.first_off + startOfLine;
			}

			ed_edit_batch_replace(&editorBatchEdit, &replacements);
		}
	} else {
		EditorBatchReplace replacements;
		replacements.size = ed_line_count(ctx);
		replacements.array = push_array(scratch.arena, EditorReplaceData, replacements.size);

		// make a replacement for each line
		for EachIndex(i, replacements.size) {
			EditorOffsetRange lineRange;
			String8 lineText;
			ed_byte_range_at_line(ctx, i, &lineRange);
			ed_string_at_range(scratch.arena, ctx, &lineRange, &lineText);
			...
			memset(newLeadingData, '\t', numTabs);
			memset(newLeadingData + numTabs, ' ', numSpaces);
			replacements.array[i].buf = str8(newLeadingData, numTabs + numSpaces);
			replacements.array[i].range.first_off = lineRange.first_off;
			replacements.array[i].range.last_off = lineRange.first_off + startOfLine;
		}

		ed_edit_batch_replace(&editorBatchEdit, &replacements);
	}

	ed_edit_batch_end(ctx, &editorBatchEdit);
	scratch_end(scratch);
}

DEF_PLUGIN_EDITOR_HOOK("Spaces to tabs", "Converts all instances of TABSTOP leading spaces to tabs", spaces_to_tabs) {
	tab_convert(ctx, 1);
}

DEF_PLUGIN_EDITOR_HOOK("Tabs to spaces", "Converts all leading tabs to TABSTOP spaces", tabs_to_spaces) {
	tab_convert(ctx, 0);
}
